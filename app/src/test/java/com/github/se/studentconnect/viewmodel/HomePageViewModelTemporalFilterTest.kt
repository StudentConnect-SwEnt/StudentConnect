...
class HomePageViewModelTemporalFilterTest {
  ...
  @Test
  fun applyFilters_privateEvent_followsTemporalFilter() = runTest {
    // Arrange - Create past and future private events
    val calendar = Calendar.getInstance()
    ...
    val pastEvent =
        Event.Private(
            uid = "private-past",
            title = "Private Past Event",
            description = "Past private event",
            start = Timestamp(calendar.time),
            end = Timestamp(calendar.time),
            location = epflLocation,
            ownerId = "owner1",
    ...
    val futureEvent =
        Event.Private(
            uid = "private-future",
            title = "Private Future Event",
            description = "Future private event",
            start = Timestamp(calendar.time),
            end = Timestamp(calendar.time),
            location = epflLocation,
            ownerId = "owner2",
    ...
    // Assert - Only future private event should be included
    val filteredEvents = viewModel.uiState.value.events
    assertEquals(1, filteredEvents.size)
    assertTrue(filteredEvents.any { it.uid == "private-future" })
    assertTrue(filteredEvents.none { it.uid == "private-past" })
  }
}

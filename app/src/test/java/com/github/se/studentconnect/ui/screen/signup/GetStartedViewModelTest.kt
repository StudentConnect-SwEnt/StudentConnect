...
class GetStartedViewModelTest {
  ...
  @Test
  fun `successful sign-in updates state with user`() = runTest(testDispatcher.scheduler) {
    // Arrange
  ...
  @Test
  fun `repository failure propagates error state`() = runTest(testDispatcher.scheduler) {
    // Arrange
    val credential = mockk<CustomCredential>()
    every { credential.type } returns
        com.google.android.libraries.identity.googleid.GoogleIdTokenCredential.Companion
  ...
  @Test
  fun `non Google credential triggers failure`() = runTest(testDispatcher.scheduler) {
    // Arrange
    ...
    val failure =
        IllegalStateException("Login failed: not a Google ID credential")
    coEvery { repository.signInWithGoogle(credential) } returns Result.failure(failure)
  ...
  @Test
  fun `cancellation exception reports cancellation`() = runTest(testDispatcher.scheduler) {
    // Arrange
    coEvery {
      credentialManager.getCredential(any<Context>(), any<GetCredentialRequest>())
    } throws
  ...
  @Test
  fun `unexpected exception surfaces message`() = runTest(testDispatcher.scheduler) {
    // Arrange
    coEvery {
      credentialManager.getCredential(any<Context>(), any<GetCredentialRequest>())
  ...
  @Test
  fun `credential exception yields descriptive error`() = runTest(testDispatcher.scheduler) {
    // Arrange
    coEvery {
      credentialManager.getCredential(any<Context>(), any<GetCredentialRequest>())
  ...
  @Test
  fun `signIn ignores additional requests while loading`() = runTest(testDispatcher.scheduler) {
    // Arrange
    val credential = mockk<CustomCredential>()
    every { credential.type } returns
        com.google.android.libraries.identity.googleid.GoogleIdTokenCredential.Companion
  ...
  @Test
  fun `clearErrorMsg removes error`() = runTest(testDispatcher.scheduler) {

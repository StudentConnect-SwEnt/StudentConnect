...
class LocationRepositoryNominatimTest {
  ...
  fun searchWorks() = runBlocking {
    ...
    assertNotNull(locations[0].name)
    assertTrue(locations[0].name!!.contains("New York"))
  }
  ...
  fun searchRateLimitingWorks() = runBlocking {
    repeat(5) {
      ...
      assertTrue(timeElapsed > 1000) // at least 1 second
    }
  }
  ...
  fun locationResultCanBeConvertedToJsonAndBack() {
    ...
    val string = Json.encodeToString<LocationRepositoryNominatim.LocationResult>(result)
    val resultDecoded = Json.decodeFromString<LocationRepositoryNominatim.LocationResult>(string)

...
class EventRepositoryFirestoreTest : FirestoreStudentConnectTest() {
    ...
    fun getAllEvents_returnsMultiple() {
        runBlocking {
            val e1 = Event.Private(repository.getNewUid(), "o", "Party", "secret", null, null, now, null, null, null, false)
            val e2 = Event.Public(repository.getNewUid(), "o2", "Hackathon", "fun", null, null, now, null, null, null, false, listOf("tech"))
    ...
    fun editEvent_withMismatchedUid_throws() {
        runBlocking {
            val e = Event.Private(repository.getNewUid(), "o", "Title", "desc", null, null, now, null, null, null, false)
    ...
    fun deleteEvent_removes() {
        runBlocking {
            val e = Event.Private(repository.getNewUid(), "o", "Temp", "d", null, null, now, null, null, null, false)
            repository.addEvent(e)
    ...
    fun addParticipant_thenGetParticipants() {
        runBlocking {
            val e = Event.Public(repository.getNewUid(), "o", "Concert", "d", null, null, now, null, null, null, false)
            repository.addEvent(e)
    ...
    fun getParticipants_whenNone_returnsEmpty() {
        runBlocking {
            val e = Event.Public(repository.getNewUid(), "o", "Empty", "d", null, null, now, null, null, null, false)
            repository.addEvent(e)
    ...
    fun eventFromDocument_withUnknownType_throws() {
        runBlocking {
            ...
            FirebaseEmulator.firestore.collection("events").document(badUid)
                .set(mapOf("type" to "nonsense")).await()

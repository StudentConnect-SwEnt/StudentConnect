...
class ProfileSettingsScreenTest {
  ...
  @Test
  fun profileSettingsScreen_countryFieldHasEditButton() {
    composeTestRule.setContent {
      MaterialTheme {
        ProfileSettingsScreen(currentUserId = testUser.userId, userRepository = repository)
      }
    ...
    // Find the country field area and verify edit button exists
    composeTestRule.onNodeWithText("Country").assertExists()

    // Edit icons should exist for the country field
    composeTestRule.onAllNodesWithContentDescription("Edit").assertCountEquals(5)
  }

  @Test
  fun profileSettingsScreen_editCountryButtonCallsCallback() {
    var callbackCalled = false
    var passedUserId: String? = null
    ...
    // Find and click the edit button for Country field
    // The Country field should be the second editable field (after Name)
    composeTestRule.onAllNodesWithContentDescription("Edit")[2].performClick()

    assert(callbackCalled)
    assert(passedUserId == testUser.userId)
  }

  @Test
  fun profileSettingsScreen_editCountryButtonPassesCorrectUserId() {
    val userId = "custom_user_id"
    val customUser = testUser.copy(userId = userId)
    repository = TestUserRepository(customUser)
    ...
    composeTestRule.setContent {
      MaterialTheme {
        ProfileSettingsScreen(
            ...
            userRepository = repository,
            onNavigateToEditNationality = { id -> passedUserId = id })
      }
    }

    composeTestRule.waitForIdle()

    // Click the edit button for Country field
    composeTestRule.onAllNodesWithContentDescription("Edit")[2].performClick()

    assert(passedUserId == userId)
  }

  @Test
  fun profileSettingsScreen_countryFieldEditableIsTrue() {
    composeTestRule.setContent {
      MaterialTheme {
        ProfileSettingsScreen(
            currentUserId = testUser.userId,
            userRepository = repository,
            onNavigateToEditNationality = {})
      }
    }

    composeTestRule.waitForIdle()

    // Verify Country field is displayed and has edit capability
    composeTestRule.onNodeWithText("Country").assertExists()
    composeTestRule.onNodeWithText("Switzerland").assertExists()

    // The edit button should be clickable (enabled)
    composeTestRule.onAllNodesWithContentDescription("Edit")[2].assertHasClickAction()
  }
